...
#
#Haswell
#./run_expr -d sbatch -M -w 60 -M '--ntasks-per-node=2' experiments/ior_cray_optimal_rd_s1.py
#KNL
#./run_expr -d sbatch -M -w 60 -M '-p knl -C quad,cache --ntasks-per-node=2' experiments/ior_cray_optimal_rd_s1.py
#
...
# The targets of IOR.
#
target_dirs = [ "/lustre/trscratch2/atorrez/nn" ]
...
#  "o" : [ "%s/%s/nn/ior.out" % ( target_dirs[0], user ) ],
  "o" : [ "%s/ior_MPIIO_2_%s.out" % ( target_dirs[0], time.mktime( datetime.datetime.now().timetuple())) ],
#
